...
function problemA() {
	...
	// promise version
	// ???
	const promisse1 = promisifiedReadFile('poem-two/stanza-01.txt').then((res) => blue(res));

	const promisse2 = promisifiedReadFile('poem-two/stanza-02.txt').then((res) => blue(res));

	Promise.all([promisse1, promisse2]).then(() => console.log('done'));
}
...
function problemB() {
	...
	const promisseArray = filenames.map((file) => promisifiedReadFile(file).then((res) => blue(res)));

	Promise.all(promisseArray).then(() => console.log('done'));
}
...
function problemC() {
	...
	const promisses = filenames.map((e) => promisifiedReadFile(e));
...
function problemE() {
	...
	function promisifiedWriteFile(filename, str) {
		...
		return new Promise(res, (rej) => {
			fs.readFile(filename, str, 'utf-8', (err) => {
				if (err) rej(err);
				res();
			});
		});
